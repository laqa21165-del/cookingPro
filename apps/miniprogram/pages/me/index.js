...
function formatCustomerBindings(bindings = []) {
    return bindings.map((item) => (Object.assign(Object.assign({}, item), { displayName: item.chefName || '未命名厨师', relatedUserId: item.chefId })));
}
function formatChefBindings(bindings = []) {
    return bindings.map((item) => (Object.assign(Object.assign({}, item), { displayName: item.customerName || '未命名顾客', relatedUserId: item.customerId })));
}
...
Page({
    ...
    async loadMe() {
        try {
            ...
            const customerBindings = formatCustomerBindings(me.bindings && me.bindings.asCustomer ? me.bindings.asCustomer : []);
            const chefBindings = formatChefBindings(me.bindings && me.bindings.asChef ? me.bindings.asChef : []);
        ...
        try {
            const res = await new Promise((resolve, reject) => {
                wx.requestSubscribeMessage({
                    tmplIds: [notification_1.WECHAT_TEMPLATE_ID],
                    success: resolve,
                    fail: reject,
                });
            });
            const status = res && res[notification_1.WECHAT_TEMPLATE_ID];
            if (status === 'reject') {
            ...
            await (0, notification_1.subscribeNotification)(notification_1.WECHAT_TEMPLATE_ID);
    ...
    async onNicknameBlur(event) {
        ...
        if (nickname === current)
            return;
        try {
            await (0, user_1.updateProfile)({ nickname });
            await this.loadMe();
        }
        catch (error) {
