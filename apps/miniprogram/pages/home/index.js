...
function formatTodoOrders(orders) {
  return (orders || [])
    .filter(function (order) { return order.status === 'pending'; })
    .slice(0, 3)
    .map(function (order) {
      return {
        id: order.id,
        shortId: String(order.id || '').slice(-4).toUpperCase(),
        itemsText: (order.items || []).map(function (sub) { return sub.name + ' x' + sub.quantity; }).join(' ・ '),
        itemCount: (order.items || []).reduce(function (sum, sub) { return sum + (sub.quantity || 0); }, 0),
...
Page({
  ...
  async onShow() {
    ...
    const topInset = menuRect ? menuRect.bottom : (windowInfo.statusBarHeight || 20) + 30;
  ...
  async bootstrap() {
    try {
      ...
      this.setData({
        ...
        hasTodoOrders: (chefOrders || []).some(function (order) { return order.status === 'pending'; }),
      });
      const tabBar = typeof this.getTabBar === 'function' ? this.getTabBar() : null;
